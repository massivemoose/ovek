...
func TestStatusNarrowFormatRendersActivityAsRecords(t *testing.T) {
	currentDeploymentID := "dep_123"
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/v1/projects/demo-app":
		...
		case "/v1/projects/demo-app/runtime":
			_ = json.NewEncoder(w).Encode(brainapi.ProjectRuntime{
				...
				App: &brainapi.ProjectRuntimeApp{
					ContainerName: "ovek-demo-app-app",
					ImageRef:      "ghcr.io/example/really-long-capsule-name:latest",
					Running:       true,
				},
			})
		case "/v1/projects/demo-app/jobs":
			_ = json.NewEncoder(w).Encode([]brainapi.Job{{
				ID:        "job_123",
				Status:    "succeeded",
				SourceRef: "ghcr.io/example/really-long-capsule-name:latest",
				CreatedAt: "2026-04-15T00:01:00Z",
			}})
		case "/v1/projects/demo-app/deployments":
			_ = json.NewEncoder(w).Encode([]brainapi.Deployment{{
	...
	var stdout strings.Builder
	var stderr strings.Builder
	exitCode := runWithStore(context.Background(), []string{"status", "--format", "narrow", "demo-app"}, &stdout, &stderr, store)
	if exitCode != 0 {
		t.Fatalf("expected exit code 0, got %d with stderr %q", exitCode, stderr.String())
	}

	output := stdout.String()
	for _, fragment := range []string{"Job: job_123", "Status: succeeded", "Deployment: dep_123", "Image: ghcr.io/example/really-long-capsule-name:latest"} {
		if !strings.Contains(output, fragment) {
			t.Fatalf("expected narrow status output to contain %q, got %q", fragment, output)
		}
	}
}
...
func TestStatusAutoFormatUsesOvekColumnsOverride(t *testing.T) {
	...
	output := stdout.String()
	if !strings.Contains(output, "Job: job_123") {
		t.Fatalf("expected OVEK_COLUMNS to force narrow activity output, got %q", output)
	}
}
